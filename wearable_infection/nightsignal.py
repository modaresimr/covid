...
def run(heartrate_file = "",step_file = "",restinghr_file = "",device = "",id="",red_threshold=4):
    ...
    if(device=="Fitbit"):
        ...
        for record in records:
            if ("time" not in record):
                ...
                rec_st = record_elements[-1].strip(' \t\n\r')
    ...
    ################################# AppleWatch #################################
        ...
        # with open(step_file  , "r") as stepCSV:
            # stepCSVReader = csv.DictReader(stepCSV,delimiter=',', skipinitialspace=True)
        
        if isinstance(step_file, pd.DataFrame):
            stepCSVReader = step_file.reset_index().astype(str).T.to_dict().values()
        else:
            stepCSVReader = pd.read_csv(step_file).T.to_dict().values()
        for step_rec in stepCSVReader:
                if step_rec['start_datetime']:
                    sdate = step_rec['start_datetime'].split(' ')
                    edate=step_rec['end_datetime'].split(' ')
                    st_start_date = sdate[0]
                    st_start_time = sdate[1]
                    st_end_date = edate[0]
                    st_end_time = edate[1]
                else:
        ...
        with open(f'output/ns/{id}/AW_rhr.csv' , "w") as rhrFile:
            ...
            if isinstance(heartrate_file, pd.DataFrame):
                hrCSVReader = heartrate_file.reset_index().astype(str).T.to_dict().values()
            else:
                hrCSVReader = pd.read_csv(heartrate_file).T.to_dict().values()
            for hr_rec in hrCSVReader:
                if 'datetime' in hr_rec:
                ...
                if (hr_start_date in dateTimes):
                    ...
                    if ( datetime.datetime.strftime(hr_time, '%H:%M') not in arrayForThisDay):
                        rhrFile.write(f'{device},{hr_start_date},{hr_start_time},{hr_value}\n')
                    # else:
