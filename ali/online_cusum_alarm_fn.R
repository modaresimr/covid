...
#### Date: 2021 June
#### Git Url: https://github.com/mwgrassgreen/Alarm/blob/master/R/online_cusum_alarm_fn.R
...
online.alarming.fn = function(peo.id.1, dir.hr, dir.step, track.par=12, gap.thres = 14, watch.type='fitbit', tune.ind=FALSE, covid.day=NULL, plot.shift.day=0, dat.saved=FALSE) {
	...
	root=paste('../output/',peo.id.1,sep='')
	root=paste('output/',sep='')
	dir.create(root, showWarnings = FALSE, recursive = TRUE)
	dir.fig = paste(root,"/figure/",sep='')
    dir.tab = paste(root,"/table/",sep='')
    dir.result = paste(root,"/result/",sep='')
    dir.clean = paste(root,"/clean/",sep='')
    dir.note = paste(root,"/note/",sep='')
		...
		for (par.ind in 1:length(par.seq)) {
			...
			for (i.chunk in 1:nrow(day.chunk)) {
				...
				if (length(hr.dates.keep) <= 28){
					...
					png(file=paste(dir.fig.train, peo.id.1,"_figure_par", par.0, '_chunk',i.chunk, ".png", sep=""), width=1700, height=800)
# width=17, height=8)
	                ...
	                if(plot.shift.day != 0) {
						...
						png(file=paste(dir.fig.train, peo.id.1,"_shifted_figure_par", par.0, '_chunk',i.chunk, ".png", sep=""), width=1700, height=800)
...
dat.format.fn = function(dir.hr, dir.step, watch.type) {
	...
	if (watch.type == 'apple') {
		for (i in 1:nrow(dat.step)) {
			...
			# print(min.intv)
			# print(as.numeric(dat.step[i, 'steps']))
		}			
	}
...
watchtype=args[4]
covidday=args[5]
online.alarming.fn(id,hr,step,watch.type=watchtype,covid.day=covidday)
